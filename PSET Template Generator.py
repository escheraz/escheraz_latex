...
if(path.exists(filename)):
	...
	if s != "y":
		exit()
s = input("Include Collaborators? (y/n): ")
t = input("Number Problems Alphabetically? (y/n): ")

if s == "y":
	if t == "y":
		output += "\\newcommand{\\problem}[1]{\\stepcounter{problemnum}  \\begin{center} \\Large \\textbf{Problem \\Alph{problemnum}} \\normalsize \\\\\\vspace{0.2cm} \\textit{Collaborators: #1}\\\\\\vspace{0.2cm}\\end{center}}"
	else:
		output += "\\newcommand{\\problem}[1]{\\stepcounter{problemnum}  \\begin{center} \\Large \\textbf{Problem \\theproblemnum} \\normalsize \\\\\\vspace{0.2cm} \\textit{Collaborators: #1}\\\\\\vspace{0.2cm}\\end{center}}"
else:
	if t == "y":
		output += "\\newcommand{\\problem}{\\stepcounter{problemnum}  \\begin{center} \\Large \\textbf{Problem \\Alph{problemnum}} \\normalsize \\\\\\vspace{0.2cm}\\end{center}}"
	else:
		output += "\\newcommand{\\problem}{\\stepcounter{problemnum}  \\begin{center} \\Large \\textbf{Problem \\theproblemnum} \\normalsize \\\\\\vspace{0.2cm}\\end{center}}"

output += """
\\newcommand\\name{Name}
\\newcommand\\email{Email}
""" + "\\newcommand\\class{" + _class + """}
\\newcommand\\psetnum{""" + psetnum + """}
\\lhead{\\name}
\\chead{\\email}
...
for i in range(numProbs):
	if s == "y":
		...
		output += "\n\\problem\n"
	n = int(input("Number of subparts for problem " + str(i + 1) + ": "))
	if n != 1:
		if t == "y":
